...
def cm_dpt(display_cmap=False):
    ...
    bounds = np.array([-10, 0, 10, 20, 30, 40, 45, 50, 55, 60, 65, 70, 75, 80])

    cmap = mcolors.LinearSegmentedColormap.from_list(
        "Dew Point", COLORS, N=len(COLORS) + 1
    )
...
def cm_rh(display_cmap=False):
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list("RH", COLORS, N=len(COLORS))
...
def cm_wind(
    display_cmap=False, continuous=False, vmin=0, vmax=140, stretch_colors=False
):
    """
    ...
    if continuous:
        ...
        BOUNDS = bounds[logic]
        cmap = mcolors.LinearSegmentedColormap.from_list(
            "Wind Speed", COLORS, N=len(COLORS) + 1
        )
...
def cm_sky(display_cmap=False):
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list(
        "Cloud Cover", COLORS, N=len(COLORS)
    )
    ...
    return {"cmap": cmap, "norm": norm}, {"label": label, "ticks": bounds}
...
def cm_precip(display_cmap=False, units="mm"):
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list(
        "Precipitation", COLORS, N=len(COLORS) + 1
    )
    norm = mcolors.BoundaryNorm(boundaries=bounds, ncolors=len(bounds))
